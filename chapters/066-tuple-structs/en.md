# Day 66: Tuple Structs — Price(f64)

## Trading Analogy

In trading, we work with many numeric values: price, quantity, volume, fee. They can all be `f64`, but **mixing them up is dangerous**:

```rust
// Dangerous! Easy to confuse arguments
fn execute_order(price: f64, quantity: f64, fee: f64) { ... }

// Which argument goes where? Easy to make mistakes!
execute_order(0.5, 42000.0, 0.001);  // Oops! Mixed up price and quantity
```

**Tuple struct** allows creating **wrapper types** that the compiler distinguishes:

```rust
struct Price(f64);
struct Quantity(f64);

// Now the compiler won't let you mix them up!
fn execute_order(price: Price, quantity: Quantity) { ... }
```

It's like each value has its own **label** — "this is a price", "this is a quantity".

## What is a Tuple Struct?

A tuple struct is a named tuple. It's similar to a regular struct, but fields have no names, and access is by index:
...
```rust
// Single element — newtype pattern
struct Price(f64);
struct Quantity(f64);
struct Volume(f64);
...
## Newtype Pattern — Error Prevention

The main advantage of tuple structs is **type safety**:
...
## Practical Example: Type-Safe Order
...
## Practical Example: PnL Calculator
...
## Comparison: Tuple vs Tuple Struct vs Struct

```rust
fn main() {
    // Regular tuple — anonymous
...
| Type | When to Use |
|------|-------------|
| Tuple | Temporary grouping of values |
| Tuple struct | Newtype pattern, type safety |
| Struct | Many fields, need named fields |

## What We Learned

| Concept | Description |
|---------|-------------|
| `struct Name(T)` | Tuple struct declaration |
| `Name(value)` | Creating an instance |
| `instance.0` | Field access |
| `let Name(x) = instance` | Destructuring |
| Newtype pattern | Wrapper for type safety |

## Homework

1. Create tuple structs for: `OrderId(u64)`, `Symbol(String)`, `Timestamp(u64)`. Implement `new()` and `value()` methods for each.

2. Write a fee calculation function that takes `Price`, `Quantity`, `FeeRate` and returns `Fee`. The compiler should prevent argument mix-ups.

3. Create a `Portfolio` struct with balances in different currencies (`USD`, `BTC`, `ETH`). Implement a `total_in_usd()` method that converts everything to dollars.

4. Implement a position size calculator with types `AccountBalance`, `RiskPercent`, `EntryPrice`, `StopLossPrice`, `PositionSize`. All inputs should be type-safe.

## Navigation

[← Previous day](../065-structs-intro/en.md) | [Next day →](../067-struct-fields/en.md)
