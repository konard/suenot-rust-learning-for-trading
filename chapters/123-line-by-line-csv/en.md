# Day 123: Line by Line — Parsing CSV Manually

## Trading Analogy

Imagine you receive a trade statement from your broker in CSV format. Each line is one trade: date, ticker, price, volume. To analyze this data, you need to read the file line by line and parse each line into parts.

CSV (Comma-Separated Values) is like an Excel spreadsheet saved in text format. Each line is a row of the table, and values are separated by commas (or semicolons).

## CSV Format for Trading Data

Typical CSV with OHLCV data:
...
## Practical Example: Loading Historical Data
...
## Escaped Fields and Quotes

In CSV, fields with special characters are usually enclosed in quotes:
...
1. Write a function `load_trades_from_csv(content: &str) -> Result<Vec<Trade>, ParseError>` that loads trades and handles all possible errors

2. Create a parser for CSV with non-standard delimiter (semicolon) and date in DD.MM.YYYY format

3. Implement a function that reads CSV with OHLCV data and finds:
   - The most volatile candle (high - low)
   - Days with volume above average
   - Consecutive bullish/bearish candle streaks

4. Write a parser for CSV with quotes where values might be missing (empty fields), and use `Option<f64>` for such cases

## Navigation

[← Previous day](../122-writing-file-trades/en.md) | [Next day →](../124-bufreader-bufwriter/en.md)
