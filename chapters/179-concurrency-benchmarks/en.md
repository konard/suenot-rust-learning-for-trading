# Day 179: Concurrency Benchmarks

## Trading Analogy

Imagine you're developing a high-frequency trading (HFT) system. You need to choose between different approaches to order processing: a single-threaded queue, a multi-threaded pool, lock-free structures, or an actor model. How do you determine which approach will be fastest for your workload?

This is similar to backtesting different trading strategies on historical data — you run each strategy under the same conditions and compare the results. In programming, this process is called **benchmarking** — measuring code performance to choose the optimal solution.

In real trading, benchmarks help answer questions like:
- How many orders per second can the system process?
- What is the latency for order matching?
- How does performance change with increasing thread count?
- What overhead does synchronization add?

## What are Concurrency Benchmarks?

Concurrency benchmarking is measuring the performance of parallel code. Key metrics include:

| Metric | Description |
|--------|-------------|
| Throughput | Number of operations per second |
| Latency | Time to complete a single operation |
| Scalability | How performance grows with thread count |
| Contention | Synchronization overhead |

## Benchmarking Tools in Rust

### 1. Criterion — The Standard Tool
...
### 2. std::time for Simple Measurements
...
## Benchmark: Single-threaded vs Multi-threaded Order Processing
...
fn process_order(order: &Order) -> f64 {
    // Simulate processing: limit checks, commission calculation
...
fn benchmark_order_processing() {
    ...
    println!("=== Order Processing Benchmark ===\n");
    println!("{:>10} | {:>8} | {:>12} | {:>10}",
             "Orders", "Threads", "Time (ms)", "Orders/sec");
    println!("{}", "-".repeat(50));
...
## Benchmark: Comparing Synchronization Primitives
...
fn main() {
    println!("=== Synchronization Primitives Benchmark ===");
    println!("Iterations: {}, Threads: {}\n", ITERATIONS, NUM_THREADS);
    ...
    println!("{:<20} | {:>12.2} | {:>15.0}",
             "RwLock (write)", rwlock_write_time.as_millis(), ops / rwlock_write_time.as_secs_f64());
    println!("{:<20} | {:>12.2} | {:>15.0}",
    ...
    println!("\nConclusion: Atomic is {:.1}x faster than Mutex",
             mutex_time.as_secs_f64() / atomic_time.as_secs_f64());
}
```

## Benchmark: Scalability with Thread Count
...
## Benchmark: Contention for Shared Resource (Order Book)
...
type Price = u64; // Price in cents to avoid float comparisons
type Quantity = u64;

#[derive(Debug, Clone)]
struct OrderBook {
    bids: BTreeMap<Price, Quantity>, // Buy orders
    asks: BTreeMap<Price, Quantity>, // Sell orders
}
...
fn main() {
    ...
    println!("=== Order Book Benchmark ===");
    println!("Operations: {}\n", iterations);
    println!("{:>8} | {:>15} | {:>15} | {:>10}",
             "Threads", "Mutex (ms)", "RwLock (ms)", "Difference");
    println!("{}", "-".repeat(60));
    ...
    println!("\nConclusion: RwLock is more efficient when reads dominate (80%+)");
}
```
...
fn main() {
    ...
    // Without black_box, the compiler might remove the computation
    black_box(result);
}
...
### 3. Consider NUMA and Cache

```rust
use std::sync::Arc;
use std::thread;

// Bad: false sharing due to adjacent atomics
struct BadCounters {
...
#[repr(align(128))] // Cache line size
...
fn main() {
    ...
    println!("Without padding: {:?}", start.elapsed());
}
```

## Practical Example: Comparing Trading System Architectures
...
// Architecture 3: Pre-partitioned Data
...
fn main() {
    ...
    println!("=== Trading System Architecture Comparison ===\n");
    println!("{:>10} | {:>15} | {:>15} | {:>15}",
             "Events", "Shared Mutex", "Channels", "Partitioned");
    println!("{}", "-".repeat(65));
    ...
    println!("\nConclusion: Pre-partitioned data is the fastest approach");
    println!("           for independent processing operations.");
}
```
...
|---------|-------------|
| Throughput | Number of operations per second |
| Latency | Response time for a single operation |
| Scalability | Performance growth with thread count |
| Criterion | Library for precise benchmarking |
| black_box | Protection against compiler optimizations |
| Warmup | Pre-measurement warm-up runs |
| False sharing | Cache line conflicts between threads |

## Homework

1. **Data Structure Benchmark**: Compare the performance of `Vec`, `VecDeque`, and `LinkedList` for an order queue when adding and removing elements from both ends. Create a graph showing the relationship with queue size.

2. **Latency Profiling**: Write a benchmark that measures not only average time but also percentiles (p50, p95, p99) for order processing. This is critical for HFT systems.

3. **Channel Comparison**: Compare the performance of `std::sync::mpsc`, `crossbeam-channel`, and `flume` for passing trade events between threads. Consider different scenarios: single producer multiple consumers (SPMC) and vice versa.

4. **Order Book Optimization**: Implement an order book using:
   - `BTreeMap` with `Mutex`
   - `BTreeMap` with `RwLock`
   - Lock-free structure (you can use `crossbeam-skiplist`)

   Measure performance at different read/write ratios (50/50, 90/10, 99/1).

## Navigation

[← Previous Day](../178-crossbeam-channels/en.md) | [Next Day →](../180-thread-pools-rayon/en.md)
