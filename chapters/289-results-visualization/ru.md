# День 289: Визуализация результатов

## Торговая аналогия

Представьте, что вы провели месяц торговли по новой стратегии и заработали 15% прибыли. Отличный результат! Но как показать его инвестору? Просто сказать "15% прибыли" недостаточно. Инвестор захочет увидеть:

- **График роста капитала** — как менялся ваш баланс день за днём
- **График просадок** — были ли периоды потерь? Насколько глубокие?
- **Распределение прибылей** — вы заработали на многих мелких сделках или на паре крупных?
- **Статистику по времени** — когда стратегия работает лучше?

Это и есть **визуализация результатов** — превращение числовых метрик в понятные графики и диаграммы, которые помогают быстро оценить эффективность торговой стратегии.

В реальном трейдинге визуализация помогает:
- Быстро оценить качество стратегии
- Обнаружить периоды просадок
- Найти паттерны в результатах
- Презентовать результаты инвесторам

## Что такое визуализация результатов?

**Визуализация результатов бэктестинга** — это процесс создания графиков, диаграмм и визуальных представлений метрик торговой стратегии для анализа и презентации.

Основные типы визуализаций:
1. **Equity Curve** (кривая капитала) — как менялся баланс со временем
2. **Drawdown Chart** (график просадок) — глубина и продолжительность убытков
3. **Trade Distribution** (распределение сделок) — гистограмма прибылей/убытков
4. **Monthly Returns** (помесячная доходность) — тепловая карта результатов
5. **Win/Loss Analysis** (анализ побед/поражений) — соотношение успешных сделок

В Rust для визуализации можно использовать библиотеки:
- **plotters** — создание графиков на чистом Rust
- **plotly** — интерактивные графики через JavaScript
- **textplots** — простые ASCII-графики в терминале

## Базовый пример: ASCII-график в терминале

Начнём с простого — создадим ASCII-график equity curve прямо в терминале:

```rust
use textplots::{Chart, Plot, Shape};

fn main() {
    // Результаты бэктестинга: баланс по дням
    ...
    println!("\n=== Метрики ===");
    println!("Начальный баланс: ${:.2}", initial_balance);
    println!("Конечный баланс:  ${:.2}", final_balance);
...
Создадим структуры для хранения результатов бэктестинга:
...
## Создание графиков с plotters

Теперь создадим настоящие графики в PNG файлах:
...
## Гистограмма распределения прибылей

Создадим гистограмму, показывающую распределение прибылей и убытков по сделкам:
...
Соберём всё вместе в рабочий пример:
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    // Создаём результаты бэктестинга
...
Для более продвинутой визуализации можно генерировать HTML с интерактивными графиками:
...
| Концепция | Описание |
|-----------|----------|
| Визуализация результатов | Превращение метрик в графики для анализа |
| Equity Curve | График изменения баланса во времени |
| Drawdown Chart | График просадок капитала |
| Распределение прибылей | Гистограмма P&L по сделкам |
| `textplots` | ASCII-графики в терминале |
| `plotters` | Создание PNG графиков |
| HTML отчёты | Интерактивные графики с plotly.js |

## Домашнее задание

1. **ASCII Dashboard**: Создайте функцию, которая выводит в терминал красивый ASCII-дашборд с:
   - Equity curve
   - Основными метриками (win rate, total return, max DD)
   - Последними 10 сделками

2. **Помесячная доходность**: Реализуйте функцию `plot_monthly_returns()`, которая создаёт тепловую карту доходности по месяцам (строки) и годам (столбцы).

3. **Сравнение стратегий**: Напишите функцию `compare_strategies()`, которая принимает результаты 2-3 стратегий и рисует их equity curves на одном графике для сравнения.

4. **Расширенный HTML отчёт**: Дополните HTML отчёт:
   - Таблицей всех сделок
   - Графиком распределения прибылей
   - Статистикой по дням недели (в какие дни недели стратегия работает лучше)
   - Графиком underwater equity (время восстановления после просадок)

## Навигация

[← Предыдущий день](../288-report-generation/ru.md) | [Следующий день →](../290-walk-forward-analysis/ru.md)
