# Day 361: A/B Strategy Testing

## Trading Analogy

Imagine you're managing a hedge fund and want to improve your trading strategy. Instead of completely replacing a working strategy with a new one (risking money loss), you can:

**A/B Testing in Trading:**
- Strategy A (control) — current proven strategy
- Strategy B (experiment) — new strategy with potential improvements
- Split capital between them and compare results

**How A/B Testing Works:**

| Aspect | Without A/B Testing | With A/B Testing |
|--------|---------------------|------------------|
| **Risk** | All capital at risk | Only a portion of capital |
| **Data** | Subjective feelings | Statistically significant metrics |
| **Rollback** | Difficult and slow | Instant switch |
| **Confidence** | Hoping for the best | Proven results |

**Real A/B Testing Scenarios:**
- Testing a new entry algorithm
- Comparing different stop-loss sizes
- Validating a new data source
- Evaluating execution latency impact

## A/B Testing Basics in Rust
...
pub enum StrategyVariant {
    Control,     // Strategy A — current
    Experiment,  // Strategy B — new
}

/// Metrics for one strategy variant
#[derive(Debug)]
pub struct VariantMetrics {
    ...
    /// Total profit in cents (offset for negative values)
    pub total_pnl_cents: AtomicU64,
    /// Maximum drawdown in cents
    pub max_drawdown_cents: AtomicU64,
    /// Sum of execution latencies in microseconds
...
impl VariantMetrics {
    pub fn new() -> Self {
        VariantMetrics {
            ...
            total_pnl_cents: AtomicU64::new(1_000_000_00), // Offset for negatives
...
/// A/B experiment for trading strategies
pub struct ABExperiment {
    pub name: String,
    pub control: Arc<VariantMetrics>,
    pub experiment: Arc<VariantMetrics>,
    pub traffic_split: f64,  // Traffic fraction for experiment (0.0 - 1.0)
...
impl ABExperiment {
    ...
    /// Select variant based on hash (deterministic)
...
fn main() {
    ...
    // Create experiment: 30% traffic to new strategy
    ...
    for trade_id in 0..200 {
        ...
        let (pnl, latency) = match variant {
            StrategyVariant::Control => {
                // Control: stable results
                let pnl = if trade_id % 3 == 0 { -15.0 } else { 10.0 };
                let latency = Duration::from_millis(50);
                (pnl, latency)
            }
            StrategyVariant::Experiment => {
                // Experiment: slightly better but less stable
...
pub struct TrafficRule {
    pub variant: String,
    pub weight: u32,           // Weight (e.g., 70 = 70%)
...
#[derive(Debug, Clone)]
pub enum Condition {
    Symbol(String),           // Only for specific symbol
    TimeRange(u32, u32),      // Only during specific hours
    AccountSize(f64, f64),    // Only for specific account size
    Always,                   // Always
}
...
fn main() {
    ...
    // Configure experiment: new algorithm for BTC
    ...
    // Configure experiment with conditions
    router.add_experiment("night_strategy", vec![
        TrafficRule {
            variant: "aggressive".to_string(),
            weight: 30,
            conditions: vec![
                Condition::TimeRange(22, 6),  // Night time
...
/// Statistical calculator for A/B tests
pub struct StatisticalCalculator;

impl StatisticalCalculator {
    /// Calculate Z-score for comparing two proportions (win rate)
    ...
    pub fn required_sample_size(
        baseline_rate: f64,       // Current win rate
        minimum_effect: f64,       // Minimum improvement to detect
        alpha: f64,                // Significance level (usually 0.05)
        power: f64,                // Statistical power (usually 0.8)
    ) -> u64 {
        // Z-values
...
pub struct ABTestAnalyzer {
    ...
    pub confidence: f64, // Confidence level for interval
}
...
fn main() {
    ...
    let sample_size = StatisticalCalculator::required_sample_size(
        0.45,   // Current win rate 45%
        0.05,   // Want to detect 5% improvement
        0.05,   // α = 0.05
        0.80,   // Power 80%
    );
...
/// Trading strategy trait — interface for all strategies
...
/// A/B testing manager for trading system
...
impl TradingABManager {
    ...
    /// Get strategy for current trade
    ...
    /// Select winner and switch to it
...
pub struct StoppingCriteria {
    /// Minimum number of trades for each variant
    pub min_trades: u64,
    /// Maximum experiment duration
    pub max_duration: Duration,
    /// Minimum significance level
    pub min_significance: f64,
    /// Stop on critical degradation
    pub stop_on_harm: bool,
    /// Harm threshold (percentage P&L degradation)
...
impl Default for StoppingCriteria {
    fn default() -> Self {
        StoppingCriteria {
            min_trades: 100,
            max_duration: Duration::from_secs(7 * 24 * 3600), // 1 week
...
impl ExperimentMonitor {
    ...
    pub fn check_stopping(&self) -> StopReason {
        ...
        // If improvement > 20% and enough data
...
| Concept | Description |
|---------|-------------|
| **A/B Testing** | Method for comparing two strategy variants |
| **Control/Experiment** | Baseline variant vs tested variant |
| **Traffic Distribution** | Splitting trades between variants |
| **Z-score** | Statistical measure of difference between groups |
| **P-value** | Probability of getting result by chance |
| **Statistical Significance** | Confidence that difference is not random |
| **Confidence Interval** | Range of probable values for true difference |
| **Stopping Criteria** | Conditions for terminating experiment |

## Practical Exercises

1. **Basic A/B Test**: Create a system for comparing two strategies:
   - Implement 50/50 traffic distribution
   - Collect metrics: win rate, P&L, average latency
   - Generate comparison report

2. **Statistical Analysis**: Extend the calculator:
   - Add t-test for comparing means
   - Implement effect size calculation
   - Add confidence interval visualization

3. **Multi-variant Testing**: Implement A/B/C/D support:
   - Traffic distribution between 4+ variants
   - Correction for multiple comparisons
   - Hierarchical testing

4. **Automation**: Create a fully automatic system:
   - Automatic experiment launch
   - Real-time monitoring
   - Automatic termination and winner selection
   - Result notifications

## Homework

1. **Production A/B System**: Develop a complete system:
   - Web interface for creating experiments
   - REST API for recording events
   - Dashboard with result visualization
   - Prometheus/Grafana integration

2. **Bayesian A/B Testing**: Implement alternative approach:
   - Bayesian probability of winning estimation
   - Dynamic traffic distribution (Multi-Armed Bandit)
   - Comparison with classical approach

3. **A/B for High-Frequency Trading**: Optimize for HFT:
   - Minimal overhead
   - Lock-free data structures
   - Microsecond measurements
   - Streaming statistics

4. **Multi-Exchange A/B Test**: Create a system for:
   - Testing on different exchanges simultaneously
   - Accounting for each exchange's specifics
   - Result aggregation
   - Cross-exchange analysis

5. **A/B with Machine Learning**: Integrate ML:
   - Automatic experiment parameter selection
   - Prediction of required sample size
   - Early winner detection
   - Adaptive traffic distribution

## Navigation

[← Previous day](../360-canary-deployments/en.md) | [Next day →](../362-post-mortem-incident-analysis/en.md)
